...
const AdminUserDetailsCard = ({ userAllInfoTypes, currentUserId }: { userAllInfoTypes: EmployyeeAllInfoType[], currentUserId: string }) => {
  ...
  return (
    <div className='shadow-md rounded-md gap-x-5 my-5 py-5 px-5 border-dashed border flex flex-col'>
        <div className='grid grid-cols-2 my-5 text-base md:text-2xl font-semibold text-gray-500 font-serif'>
          <h2>Name</h2>
          <h2>Email</h2>
          <h2>Phone</h2>
          <h2>Role</h2>
        ...
        {users.map((data, index) => (
          <div key={index} className='grid grid-cols-2 mt-10 md:mt-0 my-2 text-xs md:text-base font-semibold text-black font-serif'>
            <h2>{data.name}</h2>
            <h2>{data.email}</h2>
            <h2>{data.phone}</h2>
            {data.role === "admin" ? (
